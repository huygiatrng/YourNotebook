...
from website.AITool.src.models.modnet import MODNet


def combined_display(image, matte):
    # calculate display resolution
    w, h = image.width, image.height
    rw, rh = 800, int(h * 800 / (4 * w))
    ...
    matte = np.repeat(np.asarray(matte)[:, :, None], 4, axis=2) / 255

    foreground = orig * matte + np.full(orig.shape, 0) * (1 - matte)
